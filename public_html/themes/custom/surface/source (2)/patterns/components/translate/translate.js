  ...
  Drupal.surfaceTranslate = {
    init: function (context) {
      once('surfaceTranslateInit', '[data-drupal-selector="translate"]', context).forEach(() => {
        document.addEventListener('keyup', (e) => {
          if (
            (Drupal.surfaceTranslate.translateIsVisible() && e.key === 'Escape') ||
            e.key === 'Esc'
          ) {
            e.preventDefault();
      ...
      // Translate toggle
      once('surfaceTranslateToggle', '[data-drupal-selector="translate-button"]', context).forEach(
        (el) => {
    ...
    showTranslate: () => {
      ...
      const translateContainer = document.querySelector(
        '[data-drupal-selector="translate-container"]'
      );
